#
# Script to perform automated testing for assignment 1 of AA, semester 1 2018
#
...
#        file of "test1.in", then we should have expected files "test1.vert.exp", "test1.edge.exp", "test1.neigh.exp" and "test1.dist.exp".  
#
#
...
#
# Another example if running test2.in and using the Facebook graph as my initial one:
#> python assign1TestScript.py -v -f facebook_combined.txt   Assign1-s1234    sample     tests/test2.in
#
#
#
# Jeffrey Chan, 2016
...
def main():
    ...
    setValidImpl = set(["adjmat", "indmat", "sample"])
    ...
    sGraphTester = "GraphTester.java"
    sFriendship = "FriendshipGraph.java"
    ...
    if bHasSourceCodeDir:
        ...
        if not os.path.isfile(sFriendship): 
            shutil.copy(os.path.join(sSourceCodeDir, sFriendship), ".")
    ...
    if bCompiled:
        ...
        for (j, sInLoopFile) in enumerate(lsInFile):
            ...
            sDistOutputFile = os.path.join(sTestName + "-" + sImpl + ".dist.out")
            ...
            sDistExpectedFile = os.path.splitext(sInFile)[0] + ".dist.exp"
            ...
            sCommand = os.path.join("java " + sClassPath + " " + sExec + " " + sImpl + " " + sVertOutputFile + " " + sEdgeOutputFile + " " + sNeighOutputFile + " " + sDistOutputFile)
            if bInputFile:
                sCommand = os.path.join("java " + sClassPath + " " + sExec + " -f " + sInputFile + " " + sImpl + " " + sVertOutputFile + " " + sEdgeOutputFile + " " + sNeighOutputFile + " " + sDistOutputFile)
            print sCommand
...
#                 lCommand = os.path.join(sCodeDir, sExec + " " + sExpectedFile + ".test")
            ...
            with open(sInFile, "r") as fIn:
                ...
                if False:
                    ...
                    bDistPassed = distanceEvaluate(sDistExpectedFile, sDistOutputFile)
                    if bVertPassed and bEdgePassed and bNeighPassed and bDistPassed:
                        passedNum += 1
                        #vTestPassed[j] = True
                        ...
                        if bVerbose:
                            ...
                            if not bNeighPassed:
                                ...
                                print >> sys.stderr, ""
                            if not bDistPassed:
                                print >> sys.stderr, "Difference between shortest path distance output and expected:"
                                proc = sp.Popen("diff -y " + sDistOutputFile + " " + sDistExpectedFile, shell=True)
                                proc.communicate()
                                print >> sys.stderr, ""                                                                        
...
def edgeEvaluate(sExpectedFile, sOutputFile):
    ...
    with open(sExpectedFile, "r") as fExpected:
        for sLine in fExpected:
            ...
            sLine1 = sLine.strip()
            lFields = re.split("[\t ]*[,|\|]?[\t ]*", sLine1)
            ltExpMatches.append((lFields[0], lFields[1]))
    ...
    with open(sOutputFile, "r") as fOut:
        for sLine in fOut:
            ...
            if len(lFields) != 2:
                ...
                ltActMatches.append((lFields[0], lFields[1]))
